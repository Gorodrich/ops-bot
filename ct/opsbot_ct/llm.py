...
def run_assignment_tiebreak_job(payload: dict[str, Any], *, cli_path: str, model: str, timeout_sec: int) -> dict[str, Any]:
    ...
    prompt = (
        ...
        + "各候補のtag_match/weak_matchは必要タグとの一致率（0〜1、機械的に算出済みの参考値）です。"
        + "is_mentionedがtrueの候補は、検出元メッセージ内で名指しされていた可能性が高い人物です"
        + "（あくまで参考情報であり、必ずしも選ぶ必要はありません。最終判断はnotes等も踏まえて行ってください）。"
        + f"出力トークン数は{max_tokens}以内に収めてください。"
